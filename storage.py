...
class CacheMemcacheStrategy(object):
    ...
    def connect(self, server):
        ...
        assert len(server) > 0, "{} is not a suitable server".format(server)
        if len(server) == 1:
            clientClass = Client
            server = server[0]
        else:
    ...
    # XX Global lock here?
    # XX memcache has own statistics about hit/misses
    @property
    def statistics(self):
    ...
    def hasEntry(self, key):
        return (key in self.localCache and self.localCache[key] is not None) or self._fetchEntry(key) is not None
    ...
    def _setIgnoreExc(self, key, value):
        try:
            ...
            if self.client.ignore_exc:
                ...
                # or count errors in a statistics object
                return None
            raise
